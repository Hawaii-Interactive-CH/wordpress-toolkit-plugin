...
class AbstractEvent extends CustomPostType implements \JsonSerializable
{
    const TYPE = 'event';
    const SLUG = "events";
    ...
    {
        $fmt = datefmt_create('fr_FR', \IntlDateFormatter::FULL, \IntlDateFormatter::FULL, 'Europe/Zurich', \IntlDateFormatter::GREGORIAN, 'MMMM');
        $agenda = array();
        $date = new \DateTime();
    ...
    public static function monthlyDates($begin, $end): array
    {
        ...
        // Sort events by date
        usort($sorted, function ($a, $b) use ($begin, $end) {
            $eventA = new self($a);
            $eventB = new self($b);
